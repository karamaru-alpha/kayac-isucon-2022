...
func getFavoritesCountByPlaylistID(ctx context.Context, db connOrTx, playlistID int) (int, error) {
	...
	if err := db.GetContext(
		ctx,
		&count,
		"SELECT fav_count FROM playlist where id = ?",
		playlistID,
	); err != nil {
		return 0, fmt.Errorf(
...
func getPopularPlaylistSummaries(ctx context.Context, db connOrTx, userAccount string) ([]Playlist, error) {
	...
	if err := db.SelectContext(
		ctx,
		&playlists,
		`SELECT a.ulid, a.name, a.is_public, a.fav_count, a.song_count, a.created_at, a.updated_at, b.account, b.display_name, c.id IS NOT NULL AS is_favorited FROM playlist a JOIN user b ON a.user_account = b.account LEFT JOIN playlist_favorite c ON a.id = c.playlist_id AND c.favorite_user_account = ? WHERE a.is_public = 1 AND b.is_ban = 0 ORDER BY a.fav_count DESC LIMIT 100`,
		userAccount,
	); err != nil {
		return nil, fmt.Errorf(
...
func insertPlaylistFavorite(ctx context.Context, db connOrTx, playlistID int, favoriteUserAccount string, createdAt time.Time) error {
	...
	if _, err := db.ExecContext(
		ctx,
		"UPDATE playlist SET fav_count=fav_count+1 WHERE id = ?",
		playlistID,
	); err != nil {
		return err
	}
...
func apiPlaylistFavoriteHandler(c echo.Context) error {
	...
	if isFavorited {
		...
		if _, err := db.ExecContext(
			...
			c.Logger().Error("error decrement playlist fav_count")
			return errorResponse(c, 500, "internal server error")
		}
	}
